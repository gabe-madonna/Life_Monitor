...
#        self.coloricon = os.path.join(os.path.dirname(__file__), 'markers/%s.png')
    ...
    def draw(self, htmlfile):
        ...
        f += '<html>\n'
        f += '<head>\n'
        f += '<meta name="viewport" content="initial-scale=1.0, user-scalable=no" />\n'
        f += '<meta http-equiv="content-type" content="text/html; charset=UTF-8"/>\n'
        f += '<title>Google Maps - pygmaps </title>\n'
        f += '<script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?libraries=visualization&sensor=true_or_false"></script>\n'
...
#        f += self.write_grids()
        ...
        f += self.write_paths()
        f += self.write_shapes()
        f += self.write_heatmap()
        f += '\t}\n'
        f += '</script>\n'
        f += '</head>\n'
        f += '<body style="margin:0px; padding:0px;" onload="initialize()">\n'
        f += '\t<div id="map_canvas" style="width: 100%; height: 100%;"></div>\n'
        f += '</body>\n'
        f += '</html>\n'
        return f
    ...
    def write_grids(self):
        ...
        for line in self.grids:
            ...
            f = f + self.write_polyline(line, settings)
        return f

    def write_points(self):
        f = ""
        for point in self.points:
            f += self.write_point(point[0], point[1], point[2], point[3])
        return f
    ...
    def write_paths(self):
        ...
        for path, settings in self.paths:
            f += self.write_polyline(path, settings)
        return f

    def write_shapes(self):
        f = ""
        for shape, settings in self.shapes:
            f += self.write_polygon(shape, settings)
        return f

    # TODO: Add support for mapTypeId: google.maps.MapTypeId.SATELLITE
    def write_map(self):
        f = ""
        f += '\t\tvar centerlatlng = new google.maps.LatLng(%f, %f);\n' % (self.center[0], self.center[1])
        f += '\t\tvar myOptions = {\n'
        f += '\t\t\tzoom: %d,\n' % (self.zoom)
        f += '\t\t\tcenter: centerlatlng,\n'
        f += '\t\t\tmapTypeId: google.maps.MapTypeId.ROADMAP\n'
        f += '\t\t};\n'
        f += '\t\tvar map = new google.maps.Map(document.getElementById("map_canvas"), myOptions);\n'
        f += '\n'
        return f

    def write_point(self,lat, lon, color, title):
        f = ""
        f += '\t\tvar latlng = new google.maps.LatLng(%f, %f);\n' % (lat, lon)
        f += '\t\tvar img = new google.maps.MarkerImage(\'%s\');\n' % (self.coloricon % color)
    ...
    def write_polyline(self,path, settings):
        clickable = False
        geodesic = True
        ...
        f = ""
        f += 'var PolylineCoordinates = [\n'
        for coordinate in path:
            f += 'new google.maps.LatLng(%f, %f),\n' % (coordinate[0], coordinate[1])
        f += '];\n'
        f += '\n'

        f += 'var Path = new google.maps.Polyline({\n'
        f += 'clickable: %s,\n' % (str(clickable).lower())
        f += 'geodesic: %s,\n' % (str(geodesic).lower())
        f += 'path: PolylineCoordinates,\n'
        f += 'strokeColor: "%s",\n' % (strokeColor)
        f += 'strokeOpacity: %f,\n' % (strokeOpacity)
        f += 'strokeWeight: %d\n' % (strokeWeight)
        f += '});\n'
    ...
    def write_polygon(self, path, settings):
        clickable = False
        geodesic = True
        ...
        f += 'var polygon = new google.maps.Polygon({\n'
        f += 'clickable: %s,\n' % (str(clickable).lower())
        f += 'geodesic: %s,\n' % (str(geodesic).lower())
        f += 'fillColor: "%s",\n' % (fillColor)
        f += 'fillOpacity: %f,\n' % (fillOpacity)
    ...
    def write_heatmap(self):
        ...
        for heatmap_points, settings_string in self.heatmap_points:
            f += 'var heatmap_points = [\n'
            for heatmap_lat, heatmap_lng in heatmap_points:
                f += 'new google.maps.LatLng(%f, %f),\n' % (heatmap_lat, heatmap_lng)
            f += '];\n'
            f += '\n'
            f += 'var pointArray = new google.maps.MVCArray(heatmap_points);' + '\n'
            f += 'var heatmap;' + '\n'
